...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrfToken($_POST['csrf_token'] ?? '')) {
    $pa = $_POST['action'] ?? '';
    if ($pa === 'save') {
        $data = [sanitize($_POST['client_name']??''), sanitize($_POST['business_name']??''), sanitize($_POST['business_category']??''), sanitize($_POST['city']??''), sanitize($_POST['photo']??''), intval($_POST['rating']??5), sanitize($_POST['review']??''), sanitize($_POST['video_url']??''), isset($_POST['is_active'])?1:0, intval($_POST['sort_order']??0)];
...
<form method="POST" class="admin-form">
    ...
    <div class="admin-form-grid"><div class="admin-form-group"><label>Photo URL</label><input type="text" name="photo" value="<?= sanitize($item['photo']) ?>" class="admin-input"></div><div class="admin-form-group"><label>Rating (1-5)</label><input type="number" name="rating" value="<?= $item['rating'] ?>" min="1" max="5" class="admin-input"></div></div>
